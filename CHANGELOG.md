...
### 🔒 Security
- **The agent can no longer destroy anything without an explicit yes.** There is no delete tool, and the shell can no longer stand in for one: `rm`, `rmdir`, `unlink`, `shred`, `srm`, `trash`, `truncate` and `dd` are refused by name with an explanation. Commands that remove work are blocked even where the base command is allowed, so `git stash`, `git clean`, `git reset --hard`, `npm ci` and `make clean` all fail, as does any argument containing `--delete`, `--force`, `-rf` or `--prune`.
- **Overwriting an existing file is now treated as destruction** and asks for approval even inside a trusted workspace, because replacing a file's contents loses what was there. Creating a new file stays friction free. The approval sheet names how many lines are about to be overwritten.
- **Every mutation is snapshotted first.** Before any write or edit, the file is copied to `~/.omnidev/agent-backups` (mode `0600`, capped by count and age) and can be restored by id. New endpoints: `GET /api/agent/backups` and `POST /api/agent/backups/{id}/restore`.
- The agent's system prompt now states the no-delete rule, so the model asks the user rather than hunting for a workaround.

### 🐛 Fixed
- **Reasoning models could silently return an empty answer.** Gemma 4 spends part of the token budget on a hidden `thinking` field, and Ollama's `num_predict` caps thinking plus content together, so a modest budget could be consumed entirely by reasoning and return `""` with `done_reason: "length"` and no error. Measured on gemma4:12b: at `num_predict=60` the model produced 157 thinking tokens and no answer, while the same prompt at 200 answered correctly. Starved calls are now retried once with real headroom, and a persistently starved call raises a clear error instead of returning an empty string. This affected every non-streaming caller: codegen, vision, DevOps planning and structured generation.

### ✨ Added
- **README demo GIFs**, generated from real captured runs rather than hand-written, plus the generator at `docs/probes/make_demo_gif.py` so they can be regenerated when the numbers change.
